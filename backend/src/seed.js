...
async function main() {
    ...
    await db_1.default.codingChallenge.deleteMany({});
    ...
    const categories = await Promise.all(categoriesData.map(c => db_1.default.category.create({ data: c })));
    const getCatId = (name) => categories.find(c => c.name === name).id;
    // 2. Python Basics Course
    await db_1.default.course.create({
        data: {
    ...
    for (const challenge of pythonChallenges) {
        await db_1.default.codingChallenge.create({
            data: {
