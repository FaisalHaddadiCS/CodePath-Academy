...
export const generateTutorResponse = async (userId: string, mode: string, messages: any[], userMessage: string, context?: any, language: string = 'en') => {
...
"""`;
    ...
    // SAFE BOOLEAN DIAGNOSTICS
    console.log("=== AI DIAGNOSTICS ===");
    console.log(`GEMINI_API_KEY configured: ${!!process.env.GEMINI_API_KEY}`);
    console.log(`AI_API_KEY configured: ${!!process.env.AI_API_KEY}`);
    console.log(`selected key source: ${process.env.GEMINI_API_KEY ? 'GEMINI_API_KEY' : (process.env.AI_API_KEY ? 'AI_API_KEY' : 'none')}`);
    console.log(`selected model name: gemini-3.6-flash`);
    console.log(`backend current working directory: ${process.cwd()}`);
    console.log(`dotenv load result: ${!!process.env.GEMINI_API_KEY || !!process.env.PORT ? 'loaded' : 'not loaded'}`);
    ...
    console.log(`provider HTTP/status category: OK`);
    console.log("======================");
    ...
    console.log(`provider HTTP/status category: ERROR`);
    let errCategory = "UNKNOWN_ERROR";
    if (error.message) {
      const msg = error.message.toLowerCase();
      if (msg.includes("key") || msg.includes("auth")) errCategory = "AUTH_ERROR";
      else if (msg.includes("config")) errCategory = "CONFIG_ERROR";
      else if (msg.includes("quota")) errCategory = "QUOTA_ERROR";
      else if (msg.includes("permission") || msg.includes("403")) errCategory = "PERMISSION_ERROR";
      else if (msg.includes("model") || msg.includes("404")) errCategory = "MODEL_ERROR";
      else if (msg.includes("unavailable") || msg.includes("fetch") || msg.includes("503") || msg.includes("network")) errCategory = "PROVIDER_UNAVAILABLE";
      else if (msg.includes("parse")) errCategory = "RESPONSE_PARSE_ERROR";
    }
    console.log(`provider error category only: ${errCategory}`);
    console.log("======================");
...
export const generateQuiz = async (userId: string, topic: string, difficulty: string, numQuestions: number, language: string = 'en') => {
  const ai = getAiClient();
...
export const generateCodeReview = async (userId: string, challenge: any, code: string, executionOutput: any) => {
  const ai = getAiClient();
